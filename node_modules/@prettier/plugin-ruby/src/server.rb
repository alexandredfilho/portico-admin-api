...
trap(:QUIT) { quit = true } if Signal.list.key?('QUIT') && RUBY_ENGINE != 'jruby'
  ...
  Thread.new do
    loop do
      ...
      Thread.new(server.accept_nonblock) do |socket|
        ...
        loc = { start: { line: e.lineno, column: e.column } }
        socket.write(JSON.fast_generate(error: e.message, loc:))
      rescue StandardError => e
        begin
          socket.write(JSON.fast_generate(error: e.message))
        rescue Errno::EPIPE
...
candidates.map! do |candidate|
  Thread.new do
    ...
    candidate if JSON.parse(stdout) == 'pong' && status.exitstatus.zero?
